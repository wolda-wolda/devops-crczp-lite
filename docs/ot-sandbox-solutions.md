...
7. **Add a Debug node** (output viewer):
...
12. **Create a new tab** or delete the previous flow

13. **Add an Inject node** (same as before)

14. **Add an Exec node** and configure it with the following command:
...
15. **Add a Debug node** wired to the exec node's stdout output

16. **Wire them:** `inject → exec → debug`

17. Click **Deploy**, then trigger the **inject** node
