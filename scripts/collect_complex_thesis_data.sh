...
ssh_cmd "$EWS_IP" "python3 -c \"import socket; s=socket.socket(); s.connect(('192.168.20.10',502)); s.sendall(b'\x00\x01\x00\x00\x00\x06\x01\x06\x00\x00\x00\x00'); s.recv(1024); s.close()\""
...
echo "Copying safety override flag from EWS..."
ssh_cmd "$EWS_IP" "cat /var/log/safety_override.txt 2>/dev/null" > "$BACKUP_DIR/safety_override.txt"
